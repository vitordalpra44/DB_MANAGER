...
def saveTable(table, connection, database):
        ...
        field_lines = [f"{column_info[0]} ({column_info[1].decode('utf-8')})"
                        if isinstance(column_info[1], bytes)
                        else f"{column_info[0]} ({column_info[1]})"
        ...
        with open(f"../databases/{database}/{table}.csv", 'w', newline='') as file_csv:
                writer_csv = csv.writer(file_csv, quoting=csv.QUOTE_NONE, quotechar='')
